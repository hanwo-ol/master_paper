...
주식 데이터는 가장 접근하기 쉬운 금융 데이터입니다. **Yahoo Finance API**(yfinance 라이브러리)는 무료로 광범위한 주식 데이터를 제공하며, 실시간 및 역사적 가격, 거래량, 조정 종가, 배당금 정보를 포함합니다. 한국 시장의 경우 **pykrx 라이브러리**를 사용하여 KOSPI, KOSDAQ, KONEX 시장의 OHLCV 데이터, 시가총액, PER, PBR, 배당수익률 등 기본 재무지표를 무료로 수집할 수 있습니다.[1][2][3][4][5][6][7]
...
**적응적 자산 배분**은 시장 조건이 변할 때 포트폴리오를 동적으로 조정하는 능력입니다. 정적 배분(예: 60/40 주식/채권)은 장기 평균에 최적화되어 있지만, 단기 체제 변화에 취약합니다. MAML이나 Reptile을 사용하면 소량의 최근 데이터로 모델을 빠르게 재조정할 수 있습니다.[52][39][45][54][60][61]
